...
gulp.task('templates', function (cb) {
    async.each(Object.keys(config.masterModules), function (moduleName, cb) {
        var module = config.masterModules[moduleName];
        glob(path.join(process.cwd(), module.dir, 'client/**/*.jade'), {
            realpath: true,
            ignore: path.join(process.cwd(), module.dir, 'client/static/**')
        }, function (err, files) {
            if (err) return cb(err);
            async.each(files, function (f, cb) {
    ...
    fs.readFile('./tmp/index.jade', function (err, content) {
        ...
        try {
            var fn = jade.compile(content, {
                pretty: !config.gulpConfig.release,
                filename: path.join(process.cwd(), "tmp", 'index.jade')
            });
            var output = fn(config.gulpConfig);
...
gulp.task('requiresModule', function (cb) {
    ...
    async.each(Object.keys(config.masterModules), function (moduleName, cb) {
        var module = config.masterModules[moduleName];
        glob(path.join(process.cwd(), module.dir, 'client', '**', '*.js'), {
            realpath: true,
            ignore: [path.join(process.cwd(), module.dir, 'client', 'static', '**'), path.join(process.cwd(), module.dir, 'client', '**', '*.spec.js')]
        }, function (err, files) {
